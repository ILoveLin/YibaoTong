package com.lzyc.ybtappcal.util.rsa;
...
public final class RSAUtil {
     ...
     * 每次加密的字节数，不能超过密钥的长度值减去11
     *
     * @param data 需加密数据的string数据
     *
     * @return 加密后的string数据
     */
    public static String encryptData(String data) {
        try {
            InputStream publicIS = App.getInstance().getAssets()
                    .open("rsa_public_key.pem");
            PublicKey publicKey = loadPublicKey(publicIS);
            Cipher cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding");
            // 编码前设定编码方式及密钥
            cipher.init(Cipher.ENCRYPT_MODE, publicKey);
            // 传入编码数据并返回编码结果
            List<byte[]> list = splitString(data, 110);
            String ret = "";
            for (int i = 0; i < list.size(); i++) {
                ...
                tmp = Base64.encodeToString(cipher.doFinal(str), Base64.DEFAULT);
                tmp = tmp.replace("\n", "");
                ret += tmp;
            }
            return ret;
    ...
    @SuppressLint("NewApi")
    public static List<byte[]> splitString(String str, int length) throws UnsupportedEncodingException {
        List<byte[]> list = new ArrayList<byte[]>();
        if (length >= str.getBytes("UTF-8").length) {
            list.add(str.getBytes("UTF-8"));
        } else {
            int curIndex = 0;
            do {
                if (str.getBytes("UTF-8").length - curIndex <= length) {
                    byte[] temp = Arrays.copyOfRange(str.getBytes("UTF-8"), curIndex, str.getBytes("UTF-8").length);
                    list.add(temp);
                } else {
                    byte[] temp = Arrays.copyOfRange(str.getBytes("UTF-8"), curIndex, curIndex+length);
                    list.add(temp);
                }
                curIndex += length;
            } while (curIndex < str.getBytes("UTF-8").length);
        }
        return list;
    }
