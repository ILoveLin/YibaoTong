...
//3.这个好办，直接比较滑动前后firstVisibleItem的值就可以判断滑动方向了。
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        if (totalItemCount != 0) {   //如果ListView中的元素不为0个
            if (this.isSameRow(firstVisibleItem)) {  // 滑动距离：不超过一个item的高度
                int newScrollY = this.getTopItemScrollY(); //获取第一个可视的item的顶部坐标
