...
package com.lzyc.ybtappcal.widget.popupwindow.lib;
...
//		setBackgroundResource(R.drawable.wheel_bg);
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		if (maxLength > 0) {
			float textWidth = Math.signum(Layout.getDesiredWidth("0",
					itemsPaint));
		...
		if (label != null && label.length() > 0) {
			labelWidth = (int) Math.signum(Layout.getDesiredWidth(label,
					valuePaint));
		}
